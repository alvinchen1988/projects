...
<?php
	...
	if($numberOfRecordsInResult == '0' ){
		...
		echo "<span class='month'>".$currentMonthDisplay."</span>";
	...
	echo "<table class='mobile-schedule'>";
		$query  = "SELECT * FROM $scheduleTable WHERE month = '".$currentmonth."' ";
		$result = $mysqli->query( $query );
			echo "<tr class='t-header'><th>Date</th><th>Room</th><th>Course / Instructor</th></tr>";
	...
	while( $oneRecord = $result->fetch_assoc() ){
		...
		echo "<tr ".$bgc.$bgca.">";
		echo "<td class='date'> ".$oneRecord['date']." </td>";
		echo "<td class='room'>".$oneRecord['room']."</td>";
		echo "<td class='lecture'><span class='course'>".$oneRecord['course']."</span><span class='instructor'>".$oneRecord['instructor']."</span></td>";
		echo "</tr>";
...
function draw_calendar($month,$year){
		...
		$calendar.= '<thead class="t-header"><th>'.implode('</th><th>',$headings).'</th></thead>';
		...
		for($x = 0; $x < $running_day; $x++):
				$calendar.= '<td class="calendar-day-np"><span class="date"></span><span class="room"></span><span class="instructor"></span><span class="course"></span></td>';
				$days_in_this_week++;
		endfor;
